    ...
    <el-row>
      <el-col :span="5">
        <div class="menu-title">
          <span>系统模块权限列表</span>
          <el-button type="success" size="mini" class="fr" @click="addMod" v-if="permission.add">
      ...
      <el-col :span="10" :offset="1" v-loading="detailLoading">
        ...
        >
          <el-form-item label="分组" :label-width="formLabelWidth" prop="IS_APP">
            <el-select v-model="form.IS_APP" placeholder="请选择分组" @change="getPopTreeData">
          ...
          <el-form-item label="权限编码" :label-width="formLabelWidth" prop="POWER_CODE">
            <el-input v-model="form.POWER_CODE" :disabled="!showCancelBtn"></el-input>
          </el-form-item>
          ...
          <el-form-item label="上级模块" :label-width="formLabelWidth" prop="PARENT_MODULE_CODE">
            <div class="el-input el-input--mini" v-if="showCancelBtn||permission.update">
              <div class="el-input__inner" @click="popTreeVisible=true" v-text="PARENT_MODULE"></div>
              <el-popover
            ...
            <el-input v-model="PARENT_MODULE" style="width:178px" v-else></el-input>
          </el-form-item>
          ...
          <div class="btn-row" v-if="permission.update || permission.add">
...
export default {
  data() {
    return {
      moduleList: [],//模块菜单树
      popTreeList: [],//表单中选择父级的模块菜单树
      treeLoading: false,
      modName: "",//当前编辑的模块名称
      formLabelWidth: "80px",
      PARENT_MODULE: "",//表单中显示的父级模块
      form: {
        IS_APP: "",
        MODULE_NAME: "",
        MODULE_ID: "",
  ...
  methods: {
    ...
    renderContent(h, { node, data, store }) {
      if (data.tag == "1" || !this.permission.delete) {
        return (
